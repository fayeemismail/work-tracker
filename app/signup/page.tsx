...
export default function Signup() {
  const { user, signup, loginWithGoogle, isConfigured } = useAuth();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      success("Welcome to Pulse! Your workouts are ready.", "Account Created");
      router.push("/dashboard");
    } catch (err: unknown) {
      const firebaseError = err as { code?: string; message?: string };
      console.error(firebaseError);
      let errMsg = "Could not register account. Please try again.";
      ...
      <div className="w-full max-w-md">
        ...
        <Card className="border border-border bg-card">
          <CardHeader>
            ...
            <CardDescription>
              Start tracking your gym routines and share progress
            </CardDescription>
          </CardHeader>
          ...
          <CardFooter className="flex justify-center text-xs text-muted-foreground">
            ...
            <Link href="/login" className="text-foreground font-semibold ml-1 hover:underline">
              Sign In
            </Link>
          </CardFooter>
        </Card>
